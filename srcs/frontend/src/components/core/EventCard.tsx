  ...
  >
    ...
    <div className="p-4 flex flex-col gap-2 flex-1">
      ...
      <div className="flex items-center gap-3 text-[10px] text-white/30 mt-auto pt-2 border-t border-white/10">
        {day && month && <span className="flex items-center gap-1 shrink-0"><CalendarDays size={10} className="text-[#c8ff00]" /> {day} {month}</span>}
        {time && <span className="flex items-center gap-1 shrink-0"><Clock size={10} className="text-[#c8ff00]" /> {time}</span>}
        {duration && <span className="flex items-center gap-1 shrink-0"><Timer size={10} className="text-[#c8ff00]" /> {duration}</span>}
        {location && <span className="flex items-center gap-1 truncate"><MapPin size={10} className="text-[#c8ff00] shrink-0" /> <span className="truncate">{location}</span></span>}
      </div>
    </div>
